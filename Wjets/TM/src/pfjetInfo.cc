#include "Wjets/TM/interface/pfjetInfo.h"
...
void pfjetInfo::Fill(const edm::Event& iEvent,const  edm::EventSetup& iSetup){
  ...
  for( pfjet = pfjets.begin(); pfjet!=pfjets.end(); ++pfjet){
    ...
    //cout<<" all? "<< pfjet->bDiscriminator("pfDeepCSVJetTags:probb")<<"  "<< pfjet->bDiscriminator("pfDeepCSVJetTags:probbb")<<endl;
    //cout<<" btag "<<pfjet->bDiscriminator("pfInclusiveSecondaryVertexFinder")<<endl;
    //cout<<std::max(0.f,pfjet->bDiscriminator("combinedSecondaryVertexBJetTags"))<< " " <<std::max(0.f,pfjet->bDiscriminator("combinedInclusiveSecondaryVertexBJetTags"))<<endl;
    float btag = 0;
    btag = pfjet->bDiscriminator("pfDeepCSVJetTags:probb") + pfjet->bDiscriminator("pfDeepCSVJetTags:probbb");
...
void pfjetInfo::SetBranches(){
  ...
  AddBranch(&PFJet_DeepCSVb              ,"PFJet_DeepCSVb");
