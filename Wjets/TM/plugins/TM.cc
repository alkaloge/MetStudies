...
#include "Wjets/TM/interface/TM.h"
...
void TM::beginRun(const edm::Run& iRun, const edm::EventSetup& iSetup){
  ...
  if(hltPrescale_.init(iRun,iSetup,hltlabel_,changed)){
    ...
    if(changed){
      ...
      std::vector<std::string> list_triggers_in_run;
      list_triggers_in_run.clear();
      if(debug_){
      ...
      for(unsigned int t=0;t<ntriggers;++t){
        ...
        string string_search1 ("HLT_IsoMu24_v");
        string string_search2 ("HLT_IsoMu27_v");
	...
	if(found1!=string::npos || found2!=string::npos ){
	  list_triggers_in_run.push_back(hltname);
	}
      ...
      for(int x = 0; x< (int)list_triggers_in_run.size();x++){
        bool found = false;

        for(int i = 0; i< (int)all_triggers.size();i++){
          if(all_triggers[i]==list_triggers_in_run[x]) found = true;
        }//loop all triggers

        if(!found)
          all_triggers.push_back(list_triggers_in_run[x]);
